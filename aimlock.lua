...
if CiazwareUniversalAimbotLoaded == true then
    Notify("Deszk Gui", "Script Loaded Already", "", 3)
    return 
end
    ...
    if workspace and workspace:FindFirstChild"Teams" then 
        if getgenv().TeamCheck == true then
            if #workspace.Teams:GetChildren() == 0 then 
                ...
                SeparateNotify("Deszk Gui", "TeamCheck set to: "..tostring(getgenv().TeamCheck).." because there are no teams!", "", 3)
            end
        end
    end
end
...
Uis.InputBegan:Connect(function(Key)
    if not (Uis:GetFocusedTextBox()) then 
        ...
        if Key.KeyCode == Enum.KeyCode[AimlockToggleKey] then 
            Aimlock = not Aimlock
            Notify("Deszk Gui", "Aimlock: "..tostring(Aimlock), "", 3)
        end
    end
end)
...
SeparateNotify("Deszk Gui", "Aimbot loaded in: "..string.format("%.7f", tostring(tick() - CiazwareUniversalAimbotLoadingTime)), "", 3)
